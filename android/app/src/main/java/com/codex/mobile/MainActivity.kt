...
class MainActivity : AppCompatActivity() {
    ...
    private fun runSetup() {
        ...
        // Step 2d: Install pi_agent_rust core (canonical runtime)
        if (!serverManager.isPiCoreInstalled()) {
        ...
        // Step 2e: Install ZeroClaw compatibility layer (gateway + control UI)
        if (!serverManager.isZeroClawCompatInstalled()) {
            updateStatus("Installing ZeroClaw compatibility layer…")
            val compatOk = serverManager.installZeroClawCompat { msg -> updateDetail(msg) }
            if (!compatOk) {
                Log.w(TAG, "ZeroClaw compatibility install failed — continuing without Pi gateway")
            } else {
                updateStatus("ZeroClaw compatibility installed")
            }
        }
        ...
        // Step 7: Start Pi/ZeroClaw compatibility services
        if (serverManager.isZeroClawCompatInstalled()) {
            updateStatus("Starting Pi compatibility gateway…")
            val gatewayOk = serverManager.startPiGatewayCompat()
            if (!gatewayOk) {
